...
partial class Build
{
	Target BenchmarkDotNet => _ => _
		.Executes(() =>
		{
			...
			DotNetBuild(s => s
				.SetProjectFile(Solution.Benchmarks.Mockolate_Migration_Benchmarks)
				.SetConfiguration(Configuration.Release)
				.EnableNoLogo());

			DotNet(
				$"{Solution.Benchmarks.Mockolate_Migration_Benchmarks.Name}.dll --exporters json --filter * --artifacts \"{benchmarkDirectory}\"",
				Solution.Benchmarks.Mockolate_Migration_Benchmarks.Directory / "bin" / "Release");
		});
	...
	Target BenchmarkResult => _ => _
		...
		.Executes(async () =>
		{
			string fileContent = await File.ReadAllTextAsync(ArtifactsDirectory / "Benchmarks" / "results" /
			                                                 "Mockolate.Migration.Benchmarks.HappyCaseBenchmarks-report-github.md");
		...
		.Executes(async () =>
		{
			...
			if (int.TryParse(prNumber, out int prId))
			{
				...
				IReadOnlyList<IssueComment> comments =
					await gitHubClient.Issue.Comment.GetAllForIssue("aweXpect", "Mockolate.Migration", prId);
				long? commentId = null;
				...
				if (commentId == null)
				{
					...
					await gitHubClient.Issue.Comment.Create("aweXpect", "Mockolate.Migration", prId, body);
				}
				else
				{
					Log.Information($"Update comment:\n{body}");
					await gitHubClient.Issue.Comment.Update("aweXpect", "Mockolate.Migration", commentId.Value, body);
				}
			}
		});
